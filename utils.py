...
class RYA(Queue):
    ...
    def extend(self, items):
        #self.A.extend(sorted(items, key=lambda a:a.path_cost)) No se podría así porque solo ordeno loa hijoa del padre
        # y no toda la lista para seguir el camino más corto
        self.A.extend(items) #Solucion: primero extiendo añadiendo los hijos en la cabeza
        self.A.sort(key=lambda a:a.path_cost) # y luego ordeno la lista entera por su orden de path_cost
    def pop(self):
        e = self.A[self.start]
        self.start += 1
        if self.start > 5 and self.start > len(self.A) / 2:
